...
public class MissionControlImpl implements MissionControl {
    ...
    private Double checkAndReformMissionRating(Double missionRating) {
        return Math.min(Math.max(missionRating, MISSION_RATING_MINIMUM_VALUE), MISSION_RATING_MAXIMUM_VALUE);

    }

    private Double checkAndreformMissionBounty(Double missionBounty) {
        return Math.min(Math.max(missionBounty, MISSION_BOUNTY_MINIMUM_VALUE), MISSION_BOUNTY_MAXIMUM_VALUE);
    }
